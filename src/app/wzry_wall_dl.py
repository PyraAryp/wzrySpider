...
class WzrzWallDL:
    ...
    def __init__(self):
        ...
        if os.path.isfile(self.conf_path):
            ...
            print("Warn: The config file %s is not exit!!!" % (self.conf_path))
            sys.exit(1)
    ...
    def wzrz_wall_dl(self):
        ...
        while page_num <= self.max_page_number:
            ...
            for (i, x) in enumerate(title_lists):
                ...
                try:
                    ...
                    self.logger_handle.error('Save failed: %s', err)
                    self.logger_handle.error("The download url: %s , The filename: %s ", url, path)
                else:
                    self.logger_handle.info('Successfully saved: %s.jpg', title_lists[i])
        ...
        driver.close()
        print("OK: The program runs successfully!!!")
